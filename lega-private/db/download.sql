...
CREATE FUNCTION make_request(sid local_ega.main.stable_id%TYPE,
  ...
  RETURNS request_type AS
$make_request$ #variable_conflict use_column
DECLARE
...
BEGIN
  ...
  INSERT INTO local_ega_download.requests (file_id, user_info, client_ip, start_coordinate,
                                           end_coordinate)
  VALUES (archive_rec.file_id, uinfo, cip, scoord, ecoord)
  ON CONFLICT (id) DO NOTHING RETURNING local_ega_download.requests.id INTO rid;
