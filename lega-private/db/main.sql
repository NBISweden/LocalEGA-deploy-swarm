...
CREATE TRIGGER main_updated
  AFTER UPDATE
  ON local_ega.main
  FOR EACH ROW
EXECUTE PROCEDURE main_updated();
...
CREATE FUNCTION insert_file(inpath local_ega.main.submission_file_path%TYPE,
  ...
  RETURNS local_ega.main.id%TYPE AS
$insert_file$ #variable_conflict use_column
DECLARE
  file_id  local_ega.main.id%TYPE;
  file_ext local_ega.main.submission_file_extension%TYPE;
BEGIN
  -- Mark old ones as deprecated
  UPDATE local_ega.main
  SET status = 'DISABLED'
  WHERE submission_file_path = inpath
    AND submission_user = eid
    AND status <> 'ERROR';
...
CREATE FUNCTION finalize_file(inpath local_ega.files.inbox_path%TYPE,
  ...
  RETURNS void AS
$finalize_file$ #variable_conflict use_column
BEGIN
  ...
  SET status    = 'READY',
      stable_id = sid
  WHERE unencrypted_checksum = checksum
    AND unencrypted_checksum_type = upper(checksum_type)::local_ega.checksum_algorithm
    AND elixir_id = eid
    AND inbox_path = inpath
...
CREATE FUNCTION is_disabled(fid local_ega.main.id%TYPE)
  RETURNS boolean AS
$is_disabled$ #variable_conflict use_column
BEGIN
...
CREATE FUNCTION check_session_key_checksum(checksum local_ega.files.session_key_checksum%TYPE,
  ...
  RETURNS boolean AS
$check_session_key_checksum$ #variable_conflict use_column
BEGIN
  RETURN EXISTS(SELECT 1
                FROM local_ega.files
                WHERE session_key_checksum = checksum
                  AND session_key_checksum_type = upper(checksum_type)::local_ega.checksum_algorithm
...
$insert_error$
BEGIN
  INSERT INTO local_ega.errors (file_id, hostname, error_type, msg, from_user)
  VALUES (fid, h, etype, msg, from_user);
...
CREATE TRIGGER mark_ready
  ...
  ON local_ega.main -- using the main and not files
